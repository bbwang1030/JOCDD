...
# eta_list=[1/100,1/200,1/500,1/1000,1/2000]
...
for num_r, r in enumerate(rate):
    ...
    for num_eta,eta in enumerate(eta_list):
        ...
        for num,(name, model) in enumerate(models.items()):
            ...
            print(num_r,num_eta,num)
            F1score[num_r,num_eta,num] = F1_Score
        ...
        global_params = {
            ...
            "Value": [window_size, delta, threshold, burn_in, judge_size, X.shape[1], r, eta]
        }
        df_global_params = pd.DataFrame(global_params)

        # Save to Excel
        with pd.ExcelWriter(f"out_simu_drift_detection_results_summary_rate_{r:.3f}_{threshold:.2f}_{eta:.4f}.xlsx") as writer:
