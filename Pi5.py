...
bins = 50
window_size = 100
threshold = 25  # Threshold set to 1% of window size, default is 20
...
for outlier in range(int(n * rate)):
    ...
    mean = np.random.uniform(low=10, high=15, size=1)  # Generate random mean
    ...
    if outlier_index < len(all_Y):  # Ensure the index is within bounds
...
true_drift_points = [58, 139, 250, 480, 555, 745, 804, 1004, 1214, 1293, 1532, 1615, 2000]
results_list = []
...
for name, model in models.items():
    ...
    if name != "Ipod":
        ...
        ipcd = IPCD(X=X, Y=all_Y, window_size=window_size, threshold=threshold, delta=delta, eta=eta, burn_in=burn_in,
                    outlier_test=True)
        drift_points, outlier_points = ipcd.fit()
    end_time = time.time()
    execution_time = end_time - start_time  # Compute execution time

    judge_size = 30  # Valid range is 100 points before and after drift
...
with pd.ExcelWriter(f"new{rate:.2f}out_pi5_drift_detection_results_summary_rate.xlsx") as writer:
