...
for num_r, r in enumerate(rate):
    ...
    for num_thres,thres in enumerate(threshold):
        ...
        models = {
            ...
            "Ipod": IPCD(X=X, Y=all_Y, window_size=window_size, threshold=thres, delta=delta,eta=eta, burn_in=burn_in)
        }
        ...
        for num,(name, model) in enumerate(models.items()):
            ...
            if name !="Ipod":
                drift_points = drift_detection_with_ph(model, X, all_Y, window_size, delta, thres, burn_in)
            elif name =="Ipod":
                ipcd = IPCD(X=X, Y=all_Y, window_size=window_size, threshold=thres, delta=delta, eta=eta, burn_in=burn_in, outlier_test = True)
            ...
            print(num_r,num_thres,num)
            F1score[num_r,num_thres,num] = F1_Score

        df_results = pd.DataFrame(results_list)
        # Create global parameter table
        global_params = {
            "Parameter": ["window_size", "delta", "threshold", "burn_in", "judge_size", "X_dim", "rate"],
            "Value": [window_size, delta, thres, burn_in, judge_size, X.shape[1], r]
        }
        df_global_params = pd.DataFrame(global_params)

        # Save to Excel
        with pd.ExcelWriter(f"out_simu_drift_detection_results_summary_rate_{r:.3f}_{thres:.2f}_{eta:.4f}.xlsx") as writer:
