...
rate = [0]
threshold = [5,10,20,30,40,50] # 阈值为窗口大小的1%默认20
eta=1/500
...
F1score = np.ones((len(rate), len(threshold), 8)) * (-1)
for num_r,r in enumerate(rate):
    for num_thres,thres in enumerate(threshold):
        # shift_path = path + shift + '/'
        sum_power = []
        file = f'out_simu_drift_detection_results_summary_rate_{r:.3f}_{thres:.2f}_{eta:.4f}.xlsx'
        # 创建 CSV 读取器
        ...
        for row in f1_scores:
            F1score[num_r, num_thres, :] = f1_scores
        print(F1score)

Time = np.ones((len(rate), len(threshold), 8)) * (-1)
for num_r,r in enumerate(rate):
    for num_thres,thres in enumerate(threshold):
        # shift_path = path + shift + '/'
        sum_power = []
        ...
        # 逐行读取数据
        for row in time:
            Time[num_r, num_thres, :] = time
        # print(F1score)


model_name = ["Lasso", "MLP", "DT","Bagging","OSELM","GB", "Linear","JOCDD"]
threshold_name = ["5", "10", "20", "30", "40", "50"] # 自变量x
...
# 设置刻度位置
ax.set_xticks(np.arange(len(threshold_name)))
ax.set_yticks(np.arange(len(model_name)))
# 设置刻度标签
ax.set_xticklabels(threshold_name, fontsize="16")
ax.set_yticklabels(model_name, fontsize="16")

# 标注每一个因变量的数值到对应位置
for i in range(len(threshold_name)):
...
for num,(dr_idx, dr) in enumerate(models.items()):
    # print(colors[num])
    plt.plot(np.array(threshold), Time[0,:, num],  color=colors[num], marker=markers[num],
             linestyle=linestyles[num], label="%s" % dr)
# plt.axhline(y=sign_level, color='k')
plt.xlabel('Threshold')
# plt.ylabel('Type I Error')
plt.ylabel('Time (s)')
plt.xticks(ticks=np.array(threshold))
plt.yticks(ticks=np.linspace(0, 110, 12))
plt.savefig("Time_threshold_comp_noleg.png" , bbox_inches='tight')
plt.legend(loc=2, prop={'size': 8}, ncol=2)
plt.savefig("Time_threshold_comp.png" , bbox_inches='tight')
plt.clf()
